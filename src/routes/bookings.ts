...
router.post('/:id', auth, bookingIdValidationRules(), validate, asyncHandler(async (req: Request, res: Response) => {
...
router.patch('/:id', auth, bookingIdValidationRules(), validate, asyncHandler(async (req: Request, res: Response) => {
    const slot = await Slot.findById(req.params.id);
    if (!slot) throw new AppError('Slot not found', 404);
